...
class CreateJyOrderTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('jy_order', function (Blueprint $table) {
            ...
            $table->tinyInteger('shipping_status')->default(0)->comment('发货状态：1 待发货 2 已发货 3 已收货 4 退货');
            $table->tinyInteger('pay_status')->default(0)->comment('支付状态：1 未支付 2 支付中 3 支付成功');
            $table->string('consignee',60)->default('')->comment('收货人姓名,用户页面填写，默认取值表user_address');
            ...
            $table->decimal('bouns_price',10,2)->default(0.00)->comment('红包金额');
