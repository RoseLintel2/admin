...
@section('content')
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body panel-body-nopadding">
            ...
            <form class="form-horizontal form-bordered" action="/admin/category2/doEdit" method="post">
                {{csrf_field()}}
                    ...
                    <div class="col-sm-6">
                        <select class="form-control">
                            <option value="0">顶级分类</option>
                             @if(!empty($list))
                                @foreach ($list as $k =>$v)
                                    <option value="{{$v['id']}}" @if($info->f_id == $v['id']) selected @endif>{{str_repeat('※',$v['level'])."".$v['cate_name'] }}</option>
                                @endforeach
                             @endif
                        </select>
                    </div>
